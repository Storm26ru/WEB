...
document.getElementById("btn-start").onclick = function startCountdownTimer() {
    ...
    if (targetDateControl.value == "" || targetTimeControl.value == "") { alert("Select date and time"); return; }
...
function out(element, num)
{
    element.textContent = num;
    if (!num) return; 
    element.parentElement.style.display = 'block';
    if (element.parentElement.id == "years") document.getElementById("months").style.display = document.getElementById("days").style.display = 'block';
    if (element.parentElement.id == "months") document.getElementById("days").style.display = 'block';
}
function tickCountdown()
{
    ...
    targetDate.setHours(targetTime.getUTCHours());
    targetDate.setMinutes(targetTime.getUTCMinutes());
    targetDate.setSeconds(targetTime.getUTCSeconds());
    let difference = targetDate - now;
    let res = new Date(difference);
    if (res > 0) {
        out(document.getElementById("years-unit"), res.getUTCFullYear() - 1970);
        out(document.getElementById("months-unit"), res.getUTCMonth());
        out(document.getElementById("days-unit"), res.getUTCDate() - 1);
        document.getElementById("hours-unit").textContent = addLeadingZero(res.getUTCHours());
        document.getElementById("minutes-unit").textContent = addLeadingZero(res.getUTCMinutes());
        document.getElementById("seconds-unit").textContent = addLeadingZero(res.getUTCSeconds());
